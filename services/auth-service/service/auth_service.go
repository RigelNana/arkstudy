...
func (s *AuthServiceImpl) Register(userID uuid.UUID, rawPassword string) error {
    ...
    _, err := s.userClient.GetUserByID(context.Background(), &userpb.GetUserByIDRequest{Id: userID.String()})
    if err != nil {
...
func (s *AuthServiceImpl) getByUserID(userID uuid.UUID) (*models.Auth, error) {
    ...
    if ext, ok := s.repo.(interface{ GetByUserID(uuid.UUID) (*models.Auth, error) }); ok {
